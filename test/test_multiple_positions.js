...
contract("Protocol", function (addresses) {
  ...
  beforeEach(async function () {
    ...
    position_hash = await this.protocolContract.position_hash(
      ...
      [
        position_borrow_currency_fill_value, this.kernel_lending_currency_maximum_value,
        this.kernel_relayer_fee, this.kernel_monitoring_fee, this.kernel_rollover_fee, this.kernel_closure_fee,
  ...
  it("should fill the same kernel with two different positions each with a different nonce and fill_value within available kernel lend currency value", async function() {
    ...
    let errr = false
    try {
      await this.protocolContract.open_position(
        [
          this.lender, this.borrower, this.relayer, this.wrangler, this.BorrowToken.address, this.LendToken.address
        ],
        [
  ...
  it("no position should be overridden according to the sequence: Open 3 borrow positions, close 1st position, open new position.", async function() {
    ...
    assert.isTrue(first_position_2 === third_position_1, `1st position hash should now be ${third_position_1}, NOT ${first_position_2}`)
    assert.isTrue(second_position_2 === second_position_1, `2st position hash should now be ${second_position_1}, NOT ${second_position_2}`)
    assert.isTrue(third_position_2 === this.ZERO_ADDRESS, `3rd position hash should now be ${this.ZERO_ADDRESS}, NOT ${third_position_2}`)
    ...
    assert.isTrue(first_position_3 === third_position_1, `1st position hash should now be ${third_position_1}, NOT ${first_position_3}`)
    assert.isTrue(second_position_3 === second_position_1, `2st position hash should now be ${second_position_1}, NOT ${second_position_3}`)
