...
def test_initial_state(accounts,
    ...
    InterestPool.contribute(Web3.toWei(500, 'ether'), {'from': _pool_owner, 'gas': 300000})
    # Check total supply, name, symbol and decimals are correctly set
    assert test_token.totalSupply({'from': anyone}) == Web3.toWei(25000, 'ether')
    assert test_token.name({'from': anyone}) == _pool_name
    assert test_token.symbol({'from': anyone}) == '{0}.RF.{1}'.format(_pool_name, 'DAI')
    assert test_token.decimals({'from': anyone}) == 18
    # Check several account balances as 0
    assert test_token.balanceOf(_pool_owner, {'from': anyone}) == Web3.toWei(25000, 'ether')
    assert test_token.balanceOf(a2, {'from': anyone}) == 0
    assert test_token.balanceOf(a3, {'from': anyone}) == 0
    # Check several allowances as 0
    assert test_token.allowance(_pool_owner, _pool_owner, {'from': anyone}) == 0
    assert test_token.allowance(_pool_owner, a2, {'from': anyone}) == 0
    assert test_token.allowance(_pool_owner, a3, {'from': anyone}) == 0
    assert test_token.allowance(a2, a3, {'from': anyone}) == 0


def test_mint_and_burn(accounts, assert_tx_failed,
        Whale, Deployer, Governor,
        LST_token, Lend_token,
        get_ERC20_contract, get_ERC20_Pool_Token_contract, get_MFT_contract,
        get_PoolNameRegistry_contract, get_InterestPool_contract,
    ...
    InterestPool.contribute(Web3.toWei(500, 'ether'), {'from': _pool_owner, 'gas': 300000})
    assert test_token.balanceOf(_pool_owner, {'from': anyone}) == Web3.toWei(25000, 'ether')
    test_token.burn(Web3.toWei(25000, 'ether'), {'from': _pool_owner})
    assert test_token.balanceOf(_pool_owner, {'from': anyone}) == 0
    assert_tx_failed(lambda: test_token.burn(Web3.toWei(2, 'ether'), {'from': _pool_owner}))
    assert test_token.balanceOf(_pool_owner, {'from': anyone}) == 0
    # Test scenario were mintes 0 to _lend_token_holder, burns (check balance consistency, false burn)
    CurrencyDao.wrap(Lend_token.address, 0, {'from': _pool_owner, 'gas': 145000})
    assert test_token.balanceOf(_pool_owner, {'from': anyone}) == 0
    assert_tx_failed(lambda: test_token.burn(Web3.toWei(2, 'ether'), {'from': _pool_owner}))
    # Check that _lend_token_holder cannot mint
    assert_tx_failed(lambda: test_token.mint(_pool_owner, Web3.toWei(2, 'ether'), {'from': _pool_owner}))


def test_totalSupply(accounts, assert_tx_failed,
        Whale, Deployer, Governor,
        LST_token, Lend_token,
        get_ERC20_contract, get_ERC20_Pool_Token_contract, get_MFT_contract,
        get_PoolNameRegistry_contract, get_InterestPool_contract,
    ...
    tx_hash = InterestPool.contribute(Web3.toWei(500, 'ether'), {'from': _pool_owner, 'gas': 300000})
    assert test_token.totalSupply({'from': anyone}) == Web3.toWei(25000, 'ether')
    test_token.burn(Web3.toWei(1000, 'ether'), {'from': _pool_owner})
    assert test_token.totalSupply({'from': anyone}) == Web3.toWei(24000, 'ether')
    test_token.burn(Web3.toWei(24000, 'ether'), {'from': _pool_owner})
    assert test_token.totalSupply({'from': anyone}) == 0
    assert_tx_failed(lambda: test_token.burn(Web3.toWei(1, 'ether'), {'from': _pool_owner}))
    assert test_token.totalSupply({'from': anyone}) == 0


def test_transfer(accounts, assert_tx_failed,
        Whale, Deployer, Governor,
        LST_token, Lend_token,
        get_ERC20_contract, get_ERC20_Pool_Token_contract, get_MFT_contract,
        get_PoolNameRegistry_contract, get_InterestPool_contract,
    ...
    tx_hash = InterestPool.contribute(Web3.toWei(500, 'ether'), {'from': _pool_owner, 'gas': 300000})
    test_token.burn(Web3.toWei(1000, 'ether'), {'from': _pool_owner})
    test_token.transfer(a2, Web3.toWei(24000, 'ether'), {'from': _pool_owner})
    assert_tx_failed(lambda: test_token.burn(Web3.toWei(1, 'ether'), {'from': _pool_owner}))
    test_token.burn(Web3.toWei(24000, 'ether'), {'from': a2})
    assert_tx_failed(lambda: test_token.burn(Web3.toWei(24000, 'ether'), {'from': a2}))
...
def test_transferFrom_and_Allowance(accounts, assert_tx_failed,
    ...
    test_token.transfer(a2, 1, {'from': _pool_owner})
    test_token.burn(1, {'from': _pool_owner})
    # This should fail; no allowance or balance (0 always succeeds)
    assert_tx_failed(lambda: test_token.transferFrom(_pool_owner, a3, 1, {'from': a2}))
    test_token.transferFrom(_pool_owner, a3, 0, {'from': a2})
    # Correct call to approval should update allowance (but not for reverse pair)
    test_token.approve(a2, 1, {'from': _pool_owner})
    assert test_token.allowance(_pool_owner, a2, {'from': anyone}) == 1
    ...
    test_token.approve(_pool_owner, 1, {'from': a2})
    assert test_token.allowance(a2, _pool_owner, {'from': anyone}) == 1
    assert_tx_failed(lambda: test_token.transferFrom(a2, a3, 1, {'from': _pool_owner}))
    # 0-approve should not change balance or allow transferFrom to change balance
    test_token.transfer(a2, 1, {'from': _pool_owner})
    assert test_token.allowance(a2, _pool_owner, {'from': anyone}) == 1
    test_token.approve(_pool_owner, 0, {'from': a2})
    assert test_token.allowance(a2, _pool_owner, {'from': anyone}) == 0
    test_token.approve(_pool_owner, 0, {'from': a2})
    assert test_token.allowance(a2, _pool_owner, {'from': anyone}) == 0
    assert_tx_failed(lambda: test_token.transferFrom(a2, a3, 1, {'from': _pool_owner}))
    ...
    test_token.approve(_pool_owner, 1, {'from': a2})
    assert test_token.allowance(a2, _pool_owner, {'from': anyone}) == 1
    test_token.approve(_pool_owner, 2, {'from': a2})
    assert test_token.allowance(a2, _pool_owner, {'from': anyone}) == 2
    # Check that approving 0 then amount also works
    test_token.approve(_pool_owner, 0, {'from': a2})
    assert test_token.allowance(a2, _pool_owner, {'from': anyone}) == 0
    test_token.approve(_pool_owner, 5, {'from': a2})
    assert test_token.allowance(a2, _pool_owner, {'from': anyone}) == 5


def test_burnFrom_and_Allowance(accounts, assert_tx_failed,
        Whale, Deployer, Governor,
        LST_token, Lend_token,
        get_ERC20_contract, get_ERC20_Pool_Token_contract, get_MFT_contract,
        get_PoolNameRegistry_contract, get_InterestPool_contract,
    ...
    test_token.burn(1, {'from': _pool_owner})
    # This should fail; no allowance or balance (0 always succeeds)
    assert_tx_failed(lambda: test_token.burnFrom(_pool_owner, 1, {'from': a2}))
    test_token.burnFrom(_pool_owner, 0, {'from': a2})
    # Correct call to approval should update allowance (but not for reverse pair)
    test_token.approve(a2, 1, {'from': _pool_owner})
    assert test_token.allowance(_pool_owner, a2, {'from': anyone}) == 1
    assert test_token.allowance(a2, _pool_owner, {'from': anyone}) == 0
    ...
    assert test_token.balanceOf(a2, {'from': anyone}) == 1
    test_token.approve(_pool_owner, 1, {'from': a2})
    test_token.burnFrom(a2, 1, {'from': _pool_owner})
    # Allowance should be correctly updated after transferFrom
    assert test_token.allowance(a2, _pool_owner, {'from': anyone}) == 0
    # transferFrom with no funds should fail despite approval
    test_token.approve(_pool_owner, 1, {'from': a2})
    assert test_token.allowance(a2, _pool_owner, {'from': anyone}) == 1
    assert_tx_failed(lambda: test_token.burnFrom(a2, 1, {'from': _pool_owner}))
    # 0-approve should not change balance or allow transferFrom to change balance
    test_token.transfer(a2, 1, {'from': _pool_owner})
    assert test_token.allowance(a2, _pool_owner, {'from': anyone}) == 1
    test_token.approve(_pool_owner, 0, {'from': a2})
    ...
    assert_tx_failed(lambda: test_token.burnFrom(
        ZERO_ADDRESS, 0, {'from': _pool_owner}))


def test_raw_logs(accounts, get_log_args, assert_tx_failed,
        Whale, Deployer, Governor,
        LST_token, Lend_token,
        get_ERC20_contract, get_ERC20_Pool_Token_contract, get_MFT_contract,
        get_PoolNameRegistry_contract, get_InterestPool_contract,
    ...
    test_token = get_ERC20_Pool_Token_contract(address=InterestPool.pool_share_token({'from': anyone}))
    # _pool_owner wraps 500 Lend_token to L_lend_token
    tx = InterestPool.contribute(Web3.toWei(500, 'ether'), {'from': _pool_owner, 'gas': 300000})
    # Check that mint appropriately emits Transfer event
    args = get_log_args(tx, 'Transfer')
    assert args['_from'] == ZERO_ADDRESS
    assert args['_to'] == _pool_owner
    assert args['_value'] == Web3.toWei(25000, 'ether')

    # Check that burn appropriately emits Transfer event
    args = get_log_args(test_token.burn(1, {'from': _pool_owner}), 'Transfer')
    assert args['_from'] == _pool_owner
    assert args['_to'] == ZERO_ADDRESS
    assert args['_value'] == 1

    args = get_log_args(test_token.burn(0, {'from': _pool_owner}), 'Transfer')
    assert args['_from'] == _pool_owner
    assert args['_to'] == ZERO_ADDRESS
    ...
    args = get_log_args(test_token.transfer(
        a2, 1, {'from': _pool_owner}), 'Transfer')
    assert args['_from'] == _pool_owner
    assert args['_to'] == a2
    assert args['_value'] == 1

    args = get_log_args(test_token.transfer(
        a2, 0, {'from': _pool_owner}), 'Transfer')
    assert args['_from'] == _pool_owner
    assert args['_to'] == a2
    assert args['_value'] == 0

    # Check that approving amount emits events
    args = get_log_args(test_token.approve(_pool_owner, 1, {'from': a2}), 'Approval')
    ...
    args = get_log_args(test_token.transferFrom(
        a2, a3, 1, {'from': _pool_owner}), 'Transfer')
    assert args['_from'] == a2
    assert args['_to'] == a3
    assert args['_value'] == 1

    args = get_log_args(test_token.transferFrom(
        a2, a3, 0, {'from': _pool_owner}), 'Transfer')
    assert args['_from'] == a2
    assert args['_to'] == a3
    assert args['_value'] == 0
