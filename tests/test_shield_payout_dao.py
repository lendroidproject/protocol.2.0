...
def test_initialize(accounts, Deployer, get_ShieldPayoutDao_contract, ProtocolDaoContract):
    anyone = accounts[-1]
    ...
    assert not ShieldPayoutDaoContract.initialized({'from': anyone})
    ProtocolDaoContract.initialize_shield_payout_dao({'from': Deployer})
    assert ShieldPayoutDaoContract.initialized({'from': anyone})


def test_pause_and_unpause(accounts, Deployer, EscapeHatchManager, get_ShieldPayoutDao_contract, ProtocolDaoContract):
    anyone = accounts[-1]
    ShieldPayoutDaoContract = get_ShieldPayoutDao_contract(address=ProtocolDaoContract.daos(PROTOCOL_CONSTANTS['DAO_SHIELD_PAYOUT']))
    ProtocolDaoContract.initialize_shield_payout_dao({'from': Deployer})
    assert not ShieldPayoutDaoContract.paused({'from': anyone})
    ProtocolDaoContract.toggle_dao_pause(PROTOCOL_CONSTANTS['DAO_SHIELD_PAYOUT'], True, {'from': EscapeHatchManager})
    assert ShieldPayoutDaoContract.paused({'from': anyone})
    ProtocolDaoContract.toggle_dao_pause(PROTOCOL_CONSTANTS['DAO_SHIELD_PAYOUT'], False, {'from': EscapeHatchManager})
    assert not ShieldPayoutDaoContract.paused({'from': anyone})


def test_pause_failed_when_paused(accounts, assert_tx_failed, Deployer, EscapeHatchManager, get_ShieldPayoutDao_contract, ProtocolDaoContract):
    anyone = accounts[-1]
    ShieldPayoutDaoContract = get_ShieldPayoutDao_contract(address=ProtocolDaoContract.daos(PROTOCOL_CONSTANTS['DAO_SHIELD_PAYOUT']))
    ProtocolDaoContract.initialize_shield_payout_dao({'from': Deployer})
    ProtocolDaoContract.toggle_dao_pause(PROTOCOL_CONSTANTS['DAO_SHIELD_PAYOUT'], True, {'from': EscapeHatchManager})
...
#
#
# def test_pause_failed_when_uninitialized(accounts, assert_tx_failed, Deployer, EscapeHatchManager, get_CurrencyDao_contract, get_ShieldPayoutDao_contract, ProtocolDaoContract):
#     anyone = accounts[-1]
#     CurrencyDaoContract = get_CurrencyDao_contract(address=ProtocolDaoContract.daos(PROTOCOL_CONSTANTS['DAO_SHIELD_PAYOUT']))
#     assert_tx_failed(lambda: ProtocolDaoContract.toggle_dao_pause(PROTOCOL_CONSTANTS['DAO_SHIELD_PAYOUT'], True, {'from': EscapeHatchManager}))
#
#
...
#
#
def test_unpause_failed_when_paused(accounts, assert_tx_failed, Deployer, EscapeHatchManager, get_CurrencyDao_contract, ProtocolDaoContract):
    anyone = accounts[-1]
    CurrencyDaoContract = get_CurrencyDao_contract(address=ProtocolDaoContract.daos(PROTOCOL_CONSTANTS['DAO_SHIELD_PAYOUT']))
    ProtocolDaoContract.initialize_currency_dao({'from': Deployer})
...
#     _shield_market_hash = ShieldPayoutDaoContract.shield_market_hash(
#         Lend_token.address, Z19, Borrow_token.address, _strike_price
#     )
#     assert ShieldPayoutDaoContract.registered_shield_markets(_shield_market_hash) == False
...
#     assert ShieldPayoutDaoContract.registered_shield_markets(_shield_market_hash) == True
#
#
...
#     tx_11_hash = Lend_token.transfer(Lender, Web3.toWei(1000, 'ether'), {'from': owner})
...
#     tx_14_hash = UnderwriterPool.purchase_pool_currency(Web3.toWei(800, 'ether'), {'from': Lender})
...
#     tx_19_hash = Borrow_token.transfer(Borrower, Web3.toWei(10, 'ether'), {'from': owner})
...
#     tx_27_hash = PositionRegistry.repay_loan(position_id, {'from': Borrower})
...
#     time_travel(Z19)
#     # update price feed
#     tx_28_hash = PriceFeed.set_price(Web3.toWei(150, 'ether'), {'from': owner})
...
#     tx_29_hash = MarketDao.settle_loan_market(_loan_market_hash, {'from': _loan_market_settler, 'gas': 280000})
...
#     tx_30_hash = ShieldPayoutDaoContract.exercise_shield_currency(Lend_token.address, Z19, Borrow_token.address, _strike_price, 2, {'from': Borrower})
...
#     tx_31_hash = UnderwriterPool.exercise_u_currency(Z19, Borrow_token.address, _strike_price, 2, {'from': pool_owner})
...
#     time_travel(Z19)
#     # update price feed
#     tx_27_hash = PriceFeed.set_price(Web3.toWei(150, 'ether'), {'from': owner})
#     tx_27_receipt = w3.eth.waitForTransactionReceipt(tx_27_hash)
#     assert tx_27_receipt['status'] == 1
#     # assign one of the accounts as a _loan_market_settler
#     _loan_market_settler = w3.eth.accounts[3]
#     # _loan_market_settler initiates loan market settlement
#     tx_28_hash = MarketDao.settle_loan_market(_loan_market_hash, {'from': _loan_market_settler, 'gas': 280000})
#     tx_28_receipt = w3.eth.waitForTransactionReceipt(tx_28_hash)
#     assert tx_28_receipt['status'] == 1
...
#     tx_29_hash = Lend_token.transfer(_auctioned_collateral_purchaser, 1000 * 10 ** 18, {'from': owner})
#     tx_29_receipt = w3.eth.waitForTransactionReceipt(tx_29_hash)
#     assert tx_29_receipt['status'] == 1
...
#     tx_31_hash = Auction.purchase_for_remaining_currency({'from': _auctioned_collateral_purchaser, 'gas': 280000})
#     tx_31_receipt = w3.eth.waitForTransactionReceipt(tx_31_hash)
#     assert tx_31_receipt['status'] == 1
