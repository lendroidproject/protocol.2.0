...
#
#
# def test_set_minimum_mft_fee_interest_pool_dao(w3, Deployer, get_logs,
...
#     assert InterestPoolDao.minimum_mft_fee() == 0
#     tx_2_hash = ProtocolDao.set_minimum_mft_fee(
#         PROTOCOL_CONSTANTS['DAO_INTEREST_POOL'],
#         Web3.toWei(INTEREST_POOL_DAO_MIN_MFT_FEE, 'ether'),
#         transact={'from': Governor})
#     tx_2_receipt = w3.eth.waitForTransactionReceipt(tx_2_hash)
#     assert tx_2_receipt['status'] == 1
#     assert InterestPoolDao.minimum_mft_fee() == Web3.toWei(INTEREST_POOL_DAO_MIN_MFT_FEE, 'ether')
#
#
# def test_set_minimum_mft_fee_interest_pool_dao_fail_when_called_by_non_governor(w3, Deployer, Governor, assert_tx_failed, ProtocolDao):
#     tx_1_hash = ProtocolDao.initialize_interest_pool_dao(transact={'from': Deployer})
#     tx_1_receipt = w3.eth.waitForTransactionReceipt(tx_1_hash)
#     assert tx_1_receipt['status'] == 1
#     for account in w3.eth.accounts:
#         if account == Governor:
#             continue
#         assert_tx_failed(lambda: ProtocolDao.set_minimum_mft_fee(
#             PROTOCOL_CONSTANTS['DAO_INTEREST_POOL'],
#             Web3.toWei(INTEREST_POOL_DAO_MIN_MFT_FEE, 'ether'),
#             transact={'from': account}))


def test_set_fee_multiplier_per_mft_count_interest_pool_dao(w3, Deployer, get_logs,
    get_InterestPoolDao_contract,
    Governor, ProtocolDao):
    tx_1_hash = ProtocolDao.initialize_interest_pool_dao(transact={'from': Deployer})
    tx_1_receipt = w3.eth.waitForTransactionReceipt(tx_1_hash)
    assert tx_1_receipt['status'] == 1
    InterestPoolDao = get_InterestPoolDao_contract(address=ProtocolDao.daos(PROTOCOL_CONSTANTS['DAO_INTEREST_POOL']))
    assert InterestPoolDao.fee_multiplier_per_mft_count(0) == 0
    tx_2_hash = ProtocolDao.set_fee_multiplier_per_mft_count(
        PROTOCOL_CONSTANTS['DAO_INTEREST_POOL'], Web3.toWei(0, 'ether'),
        Web3.toWei(INTEREST_POOL_DAO_FEE_MULTIPLIER_PER_MFT_COUNT, 'ether'),
        transact={'from': Governor})
    tx_2_receipt = w3.eth.waitForTransactionReceipt(tx_2_hash)
    assert tx_2_receipt['status'] == 1
    assert InterestPoolDao.fee_multiplier_per_mft_count(0) == Web3.toWei(INTEREST_POOL_DAO_FEE_MULTIPLIER_PER_MFT_COUNT, 'ether')


# def test_set_fee_multiplier_per_mft_count_interest_pool_dao_fail_when_called_by_non_governor(w3, Deployer, Governor, assert_tx_failed, ProtocolDao):
#     tx_1_hash = ProtocolDao.initialize_interest_pool_dao(transact={'from': Deployer})
#     tx_1_receipt = w3.eth.waitForTransactionReceipt(tx_1_hash)
#     assert tx_1_receipt['status'] == 1
#     for account in w3.eth.accounts:
