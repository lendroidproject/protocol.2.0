...
def test_totalSupply(w3, assert_tx_failed, get_ERC20_contract):
    ...
    test_token.mint(a1, 0, transact={'from': minter})
    assert test_token.totalSupply() == 0


def test_transfer(w3, assert_tx_failed, get_ERC20_contract):
    minter, a1, a2 = w3.eth.accounts[0:3]
    test_token = get_ERC20_contract(
        'TEST Support Token', 'TST', 18, 0
    )
    ...
    test_token.mint(a1, 2, transact={'from': minter})
    test_token.burn(1, transact={'from': a1})
    test_token.transfer(a2, 1, transact={'from': a1})
    assert_tx_failed(lambda: test_token.burn(1, transact={'from': a1}))
    test_token.burn(1, transact={'from': a2})
    assert_tx_failed(lambda: test_token.burn(1, transact={'from': a2}))
    # Ensure transfer fails with insufficient balance
    assert_tx_failed(lambda: test_token.transfer(a1, 1, transact={'from': a2}))
    # Ensure 0-transfer always succeeds
    test_token.transfer(a1, 0, transact={'from': a2})


def test_maxInts(w3, assert_tx_failed, get_ERC20_contract):
    minter, a1, a2 = w3.eth.accounts[0:3]
    test_token = get_ERC20_contract(
        'TEST Support Token', 'TST', 18, 0
    )
    test_token.mint(a1, MAX_UINT256, transact={'from': minter})
    assert test_token.balanceOf(a1) == MAX_UINT256
    assert_tx_failed(lambda: test_token.mint(a1, 1, transact={'from': a1}))
    assert_tx_failed(lambda: test_token.mint(a1, MAX_UINT256, transact={'from': a1}))
    # Check that totalSupply cannot overflow, even when mint to other account
    assert_tx_failed(lambda: test_token.mint(a2, 1, transact={'from': minter}))
    # Check that corresponding mint is allowed after burn
    test_token.burn(1, transact={'from': a1})
    test_token.mint(a2, 1, transact={'from': minter})
    assert_tx_failed(lambda: test_token.mint(a2, 1, transact={'from': minter}))
    test_token.transfer(a1, 1, transact={'from': a2})
    # Assert that after obtaining max number of tokens, a1 can transfer those but no more
    assert test_token.balanceOf(a1) == MAX_UINT256
    test_token.transfer(a2, MAX_UINT256, transact={'from': a1})
    assert test_token.balanceOf(a2) == MAX_UINT256
    assert test_token.balanceOf(a1) == 0
    # [ next line should never work in EVM ]
    with pytest.raises(ValidationError):
        test_token.transfer(a1, MAX_UINT256 + 1, transact={'from': a2})
    # Check approve/allowance w max possible token values
    assert test_token.balanceOf(a2) == MAX_UINT256
    test_token.approve(a1, MAX_UINT256, transact={'from': a2})
    test_token.transferFrom(a2, a1, MAX_UINT256, transact={'from': a1})
    assert test_token.balanceOf(a1) == MAX_UINT256
    assert test_token.balanceOf(a2) == 0
    # Check that max amount can be burned
    test_token.burn(MAX_UINT256, transact={'from': a1})
    assert test_token.balanceOf(a1) == 0


def test_transferFrom_and_Allowance(w3, assert_tx_failed, get_ERC20_contract):
    minter, a1, a2, a3 = w3.eth.accounts[0:4]
    test_token = get_ERC20_contract(
        'TEST Support Token', 'TST', 18, 0
    )
    assert_tx_failed(lambda: test_token.burn(1, transact={'from': a2}))
    test_token.mint(a1, 1, transact={'from': minter})
    test_token.mint(a2, 1, transact={'from': minter})
    test_token.burn(1, transact={'from': a1})
    # This should fail; no allowance or balance (0 always succeeds)
    assert_tx_failed(lambda: test_token.transferFrom(a1, a3, 1, transact={'from': a2}))
    test_token.transferFrom(a1, a3, 0, transact={'from': a2})
    # Correct call to approval should update allowance (but not for reverse pair)
    test_token.approve(a2, 1, transact={'from': a1})
    ...
    assert_tx_failed(lambda: test_token.transferFrom(a1, a3, 1, transact={'from': a3}))
    assert test_token.balanceOf(a2) == 1
    test_token.approve(a1, 1, transact={'from': a2})
    test_token.transferFrom(a2, a3, 1, transact={'from': a1})
    ...
    assert_tx_failed(lambda: test_token.transferFrom(a2, a3, 1, transact={'from': a1}))
    # 0-approve should not change balance or allow transferFrom to change balance
    test_token.mint(a2, 1, transact={'from': minter})
    assert test_token.allowance(a2, a1) == 1
    test_token.approve(a1, 0, transact={'from': a2})
    assert test_token.allowance(a2, a1) == 0
    test_token.approve(a1, 0, transact={'from': a2})
    assert test_token.allowance(a2, a1) == 0
    assert_tx_failed(lambda: test_token.transferFrom(a2, a3, 1, transact={'from': a1}))
    ...
    test_token.approve(a1, 1, transact={'from': a2})
    assert test_token.allowance(a2, a1) == 1
    test_token.approve(a1, 2, transact={'from': a2})
    assert test_token.allowance(a2, a1) == 2
    # Check that approving 0 then amount also works
    test_token.approve(a1, 0, transact={'from': a2})
    assert test_token.allowance(a2, a1) == 0
    test_token.approve(a1, 5, transact={'from': a2})
    assert test_token.allowance(a2, a1) == 5


def test_burnFrom_and_Allowance(w3, assert_tx_failed, get_ERC20_contract):
    minter, a1, a2, a3 = w3.eth.accounts[0:4]
    test_token = get_ERC20_contract(
        'TEST Support Token', 'TST', 18, 0
    )
    ...
    test_token.burn(1, transact={'from': a1})
    # This should fail; no allowance or balance (0 always succeeds)
    assert_tx_failed(lambda: test_token.burnFrom(a1, 1, transact={'from': a2}))
    test_token.burnFrom(a1, 0, transact={'from': a2})
    # Correct call to approval should update allowance (but not for reverse pair)
    test_token.approve(a2, 1, transact={'from': a1})
    assert test_token.allowance(a1, a2) == 1
    assert test_token.allowance(a2, a1) == 0
    ...
    assert test_token.balanceOf(a2) == 1
    test_token.approve(a1, 1, transact={'from': a2})
    test_token.burnFrom(a2, 1, transact={'from': a1})
    # Allowance should be correctly updated after transferFrom
    assert test_token.allowance(a2, a1) == 0
    # transferFrom with no funds should fail despite approval
    test_token.approve(a1, 1, transact={'from': a2})
    assert test_token.allowance(a2, a1) == 1
    assert_tx_failed(lambda: test_token.burnFrom(a2, 1, transact={'from': a1}))
    # 0-approve should not change balance or allow transferFrom to change balance
    test_token.mint(a2, 1, transact={'from': minter})
    assert test_token.allowance(a2, a1) == 1
    test_token.approve(a1, 0, transact={'from': a2})
...
def test_raw_logs(w3, get_log_args, get_ERC20_contract):
    minter, a1, a2, a3 = w3.eth.accounts[0:4]
    test_token = get_ERC20_contract(
        'TEST Support Token', 'TST', 18, 0
    )
    ...
    args = get_log_args(
        test_token.mint(a1, 2, transact={'from': minter}), test_token, 'Transfer')
    assert args._from == ZERO_ADDRESS
    assert args._to == a1
    assert args._value == 2

    args = get_log_args(
        test_token.mint(a1, 0, transact={'from': minter}), test_token, 'Transfer')
    assert args._from == ZERO_ADDRESS
    assert args._to == a1
    assert args._value == 0

    # Check that burn appropriately emits Transfer event
    args = get_log_args(test_token.burn(1, transact={'from': a1}), test_token, 'Transfer')
    assert args._from == a1
    assert args._to == ZERO_ADDRESS
    assert args._value == 1

    args = get_log_args(test_token.burn(0, transact={'from': a1}), test_token, 'Transfer')
    assert args._from == a1
    assert args._to == ZERO_ADDRESS
    assert args._value == 0
    ...
    args = get_log_args(test_token.transfer(
        a2, 1, transact={'from': a1}), test_token, 'Transfer')
    assert args._from == a1
    assert args._to == a2
    assert args._value == 1

    args = get_log_args(test_token.transfer(
        a2, 0, transact={'from': a1}), test_token, 'Transfer')
    assert args._from == a1
    assert args._to == a2
    assert args._value == 0

    # Check that approving amount emits events
    args = get_log_args(test_token.approve(a1, 1, transact={'from': a2}), test_token, 'Approval')
    ...
    args = get_log_args(test_token.transferFrom(
        a2, a3, 1, transact={'from': a1}), test_token, 'Transfer')
    assert args._from == a2
    assert args._to == a3
    assert args._value == 1

    args = get_log_args(test_token.transferFrom(
        a2, a3, 0, transact={'from': a1}), test_token, 'Transfer')
    assert args._from == a2
    assert args._to == a3
    assert args._value == 0
